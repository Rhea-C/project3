...
def get_user_tweets(user):
	if user in CACHE_DICTION:
		...
		user_data= api.user_timeline(screen_name= user, count = 20)
		
		CACHE_DICTION[user]= user_data
...
cur.execute("CREATE TABLE Tweets (tweet_id TEXT NOT NULL PRIMARY KEY UNIQUE, text_1 TEXT, user_posted TEXT, retweets INTEGER, time_posted DATETIME, FOREIGN KEY(user_posted) REFERENCES Users(user_id))")
...
qu1= "INSERT INTO Tweets VALUES (?, ?, ?, ?, ?)"
tweet_spec= []
for tweet in umich_tweets: 
	tweet_spec.append((tweet["id_str"], tweet["text"], tweet["user"]["id_str"], tweet["created_at"], tweet["retweet_count"]))
for tup in tweet_spec:
	cur.execute(qu1, tup)


qu2= "INSERT OR IGNORE INTO Users VALUES (?, ?, ?, ?)"
user_spec= []
for tweet in umich_tweets:
	user_spec.append((tweet["user"]["id_str"], tweet["user"]["screen_name"], tweet["user"]["favourites_count"], tweet["user"]["description"]))

for thing in user_spec:
	cur.execute(qu2, thing)

user_mentions= []
for tweet in umich_tweets:
	for lst in tweet["entities"]["user_mentions"]:
		user_info= api.get_user(user_id= lst["id_str"])
		user_mentions.append((user_info["id_str"], user_info["screen_name"], user_info["favourites_count"], user_info["description"]))
			
for xx in user_mentions:
	cur.execute(qu2, xx)
...
cur.execute(q2)
screen_names= [str(x)[2:-3] for x in cur.fetchall()]


# Make a query to select all of the tweets (full rows of tweet information)
# that have been retweeted more than 10 times. Save the result 
# (a list of tuples, or an empty list) in a variable called retweets.
q3 = "SELECT retweets FROM Tweets WHERE retweets > 10"
cur.execute(q3)
...
q4 = "SELECT description FROM Users WHERE num_fav > 500"
cur.execute(q4)
favorites = [str(x)[2:-3] for x in cur.fetchall()]


# Make a query using an INNER JOIN to get a list of tuples with 2 
# elements in each tuple: the user screenname and the text of the 
# tweet. Save the resulting list of tuples in a variable called joined_data2.
q5 = "SELECT screen_name, text_1 FROM Users as u INNER JOIN Tweets as t ON u.user_id = t.user_posted"
cur.execute(q5)
...
q6 = "SELECT screen_name, text_1 FROM Users as u INNER JOIN Tweets as t ON u.user_id = t.user_posted ORDER BY t.retweets DESC"
cur.execute(q6)
